...
describe("GET /", () => {
	test("works", async () => {
		...
		expect(resp.body).toEqual({
			subcategories: [
				{ id: expect.any(Number), name: "testSubcategory", categoryId: testCategoryIds[0], userId: testUserIds[0] },
				{ id: expect.any(Number), name: "testSubcategory2", categoryId: testCategoryIds[0], userId: testUserIds[0] },
			],
		});
	...
	test("works when a user_id is passed into req.body", async () => {
		...
		expect(resp.body).toEqual({
			subcategories: [
				{ id: expect.any(Number), name: "testSubcategory", categoryId: testCategoryIds[0], userId: testUserIds[0] },
				{ id: expect.any(Number), name: "testSubcategory2", categoryId: testCategoryIds[0], userId: testUserIds[0] },
				{ id: expect.any(Number), name: "testSubcategory3", categoryId: testCategoryIds[1], userId: testUserIds[1] },
			],
		});
...
describe("PATCH /subcategories/:subcategory_id", () => {
	test("works", async () => {
		...
		const resp = await request(app)
			.patch(`/subcategories/${testSubcategoryIds[2]}`)
			.set("authorization", `Bearer ${u1token}`)
			.send(data);
...
describe("DELETE /:subcategory_id", () => {
	test("works", async () => {
		const resp = await request(app)
			.delete(`/subcategories/${testSubcategoryIds[0]}`)
			.set("authorization", `bearer ${u1token}`);
