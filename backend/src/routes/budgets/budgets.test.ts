...
describe('GET /budgets/:budget_id', () => {
  test('works with admin/correct user ', async () => {
    const resp = await request(app).get(`/budgets/${testBudgetIds[0]}?userId=${testUserIds[0]}`).set('authorization', `Bearer ${u1token}`);
  ...
  test('works with admin/not correct user', async () => {
    const resp = await request(app).get(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u1token}`);
  ...
  test('works with not admin/correct user', async () => {
    const resp = await request(app).get(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u2token}`);
    expect(resp.body).toEqual({
        budget: {
      id: testBudgetIds[2],
      userId: testUserIds[1],
  ...
  test('throw error if not admin and not correct user', async () => {
    const resp = await request(app).get(`/budgets/${testBudgetIds[0]}?userId=${testUserIds[0]}`).set('authorization', `Bearer ${u2token}`);
...
describe("PATCH /budgets/:budget_id", () => {
    test('works when admin/not correct user', async () => {
        ...
        const resp = await request(app).patch(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u1token}`).send(data);
        expect(resp.status).toEqual(200);
        expect(resp.body).toEqual({
            budget: {
                id: expect.any(Number),
    ...
    test("works when not admin/correct user", async () => {
        ...
        const resp = await request(app).patch(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u2token}`).send(data);
        expect(resp.status).toEqual(200);
        expect(resp.body).toEqual({
            budget: {
                id: expect.any(Number),
    ...
    test("throws error when not admin/not correct user", async () => {
        ...
        const resp = await request(app).patch(`/budgets/${testBudgetIds[0]}?userId=${testUserIds[0]}`).set('authorization', `Bearer ${u2token}`).send(data);
        expect(resp.status).toEqual(401);
        expect(resp.body).toEqual({ error: { message: 'Unauthorized', status: 401 } });
    })
...
describe("DELETE /budgets/:budget_id", () => {
    test('works when admin/not correct user', async () => { 
        const resp = await request(app).delete(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u1token}`);
        expect(resp.body).toEqual({message: "Budget deleted"})
    })
    test('works when not admin/correct user', async () => { 
        const resp = await request(app).delete(`/budgets/${testBudgetIds[2]}?userId=${testUserIds[1]}`).set('authorization', `Bearer ${u2token}`);
        expect(resp.body).toEqual({message: "Budget deleted"})
    })
    test('throws error when not admin/ not correct user', async () => { 
        const resp = await request(app).delete(`/budgets/${testBudgetIds[1]}?userId=${testUserIds[0]}`).set('authorization', `Bearer ${u2token}`);
        expect(resp.body).toEqual({ error: { message: 'Unauthorized', status: 401 } });
    })
})
